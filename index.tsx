...
const App = () => {
  ...
  useEffect(() => {
    fetchNews();
    const interval = setInterval(fetchNews, 30000); // Обновление каждые 30 сек
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-5 duration-500">
            {news.map((item) => (
              <div key={item.id} className="bg-[#0f0f11] border border-white/5 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
                ...
                <p className="text-[15px] leading-relaxed text-gray-200 mb-6 font-medium">{item.content}</p>
                <button 
          ...
          <div className="space-y-6 animate-in zoom-in-95 duration-300">
            <div className="bg-[#0f0f11] rounded-[2.5rem] p-8 border border-white/5 shadow-2xl">
              ...
              <textarea 
                ...
                className="w-full h-48 bg-black/50 border border-white/10 rounded-3xl p-6 text-sm outline-none focus:border-blue-500/50 transition-all resize-none mb-6"
              />
              <button 
                onClick={handleManualRewrite}
                disabled={isAiLoading}
                className={`w-full py-5 rounded-3xl font-black text-sm uppercase tracking-widest shadow-xl transition-all active:scale-95 ${isAiLoading ? 'bg-gray-800 text-gray-500' : 'bg-blue-600 text-white shadow-blue-500/20'}`}
              >
